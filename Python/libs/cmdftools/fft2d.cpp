...
void fft2d::fft8_2d_v(float *horz_fft_r, float *horz_fft_i, float *fft_r, float *fft_i)
{
	...
	float vec_r[bsize], vec_i[bsize];

	for (int j = 0; j<bsize; j++)
	{
		vec_r[j] = horz_fft_r[j * bsize];
	}
	...
	for (int k = 1; k < 4; k++)
	{
		...
		fftx8_1D_complex_horz(vec_r, vec_i, fft_r + k * bsize, fft_i + k * bsize);
	}

	for (int j = 0; j<bsize; j++)
	{
		vec_r[j] = horz_fft_r[j * bsize + 4];
	}
...
void fft2d::fft16_2d_v(float *horz_fft_r, float *horz_fft_i, float *fft_r, float *fft_i)
{
	const int bsize = 16;
	...
	for (int k = 1; k < 8; k++)
	{
		...
		fftx16_1D_complex_horz(vec_r, vec_i, fft_r + k * bsize, fft_i + k * bsize);
	}

	for (int j = 0; j<bsize; j++)
	{
		vec_r[j] = horz_fft_r[j * bsize + 8];
	}
